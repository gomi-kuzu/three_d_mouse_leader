...
slider_bottom = 0.30 if not args.ros2 else 0.04
gs = gridspec.GridSpec(2, 3, fig,
                       top=0.91, bottom=slider_bottom,
                       hspace=0.48, wspace=0.28)

ax_sv    = fig.add_subplot(gs[0, 0])                   # 特異値バーチャート
ax_ell   = fig.add_subplot(gs[0, 1], projection='3d')  # 可操作度楕円体 (3D)
ax_ell2d = fig.add_subplot(gs[0, 2])                   # 楕円投影 XY+XZ (2D)
ax_ns    = fig.add_subplot(gs[1, 0])                   # 最小右特異ベクトル
ax_pos   = fig.add_subplot(gs[1, 1])                   # 位置部分特異値
ax_comb  = fig.add_subplot(gs[1, 2])                   # メトリクス + 関節角度
...
def draw_ellipsoid(ax, a: dict):
    """可操作度楕円体を 3D 表示 (毎フレームクリア、視点維持)"""
    elev, azim = ax.elev, ax.azim   # 視点を保存
    ax.clear()
    sp = a['s_pos'][:3]
    Up = a['U_pos'][:, :3]
    ...
    S  = np.array([np.outer(np.cos(u), np.sin(v)),
                   np.outer(np.sin(u), np.sin(v)),
                   np.outer(np.ones_like(u), np.cos(v))])  # (3, 18, 12)
    E  = np.einsum('ij,jkl->ikl', Up @ np.diag(sp) * sc, S)
    ax.plot_wireframe(E[0], E[1], E[2], color='steelblue', alpha=0.45,
                      linewidth=1.2, rstride=2, cstride=2)
    c3 = ['#e74c3c', '#3498db', '#2ecc71']
    for k in range(3):
        vk = Up[:, k] * sp[k] * sc
        ax.quiver(0, 0, 0, vk[0], vk[1], vk[2], color=c3[k],
                  linewidth=2, arrow_length_ratio=0.15)
        ax.text(vk[0] * 1.35, vk[1] * 1.35, vk[2] * 1.35,
                f'σ{k+1}={sp[k]:.3f}', fontsize=7, color=c3[k])
    lim = max(sp[0] * sc * 1.4, 0.05)
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
    ax.set_zlim(-lim, lim)
    ax.set(title='可操作度楕円体\n(マウスで回転可)',
           xlabel='X', ylabel='Y', zlabel='Z')
    ax.view_init(elev=elev, azim=azim)  # 視点を復元

def draw_ellipses_2d(ax, a: dict):
    """XY 面・XZ 面の楕円を X 軸共有で重ね描き (毎フレームクリア)"""
    ax.clear()
    sp   = a['s_pos'][:3]
    Up   = a['U_pos'][:, :3]
    sc   = 0.4 / max(sp[0], 1e-8)
    ...
    lim  = 0.0
    for proj, col, lbl in [
        (np.array([[1., 0., 0.], [0., 1., 0.]]), '#2980b9', 'XY (縦=Y)'),
        (np.array([[1., 0., 0.], [0., 0., 1.]]), '#e67e22', 'XZ (縦=Z)'),
    ]:
        A           = proj @ Up @ np.diag(sp)
        Ua2, sa2, _ = np.linalg.svd(A, full_matrices=False)
        ell         = Ua2 @ np.diag(sa2) @ circ * sc
        ax.plot(ell[0], ell[1], '-', color=col, lw=2.5, alpha=0.95, label=lbl)
        ax.fill(ell[0], ell[1], alpha=0.06, color=col)
        lim = max(lim, sa2[0] * sc * 1.5)
    lim = max(lim, 0.05)
    ...
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
    ax.set(title='楕円投影 (X 軸共有)\nXY 面=青  XZ 面=橙',
           xlabel='X 方向', ylabel='Y / Z 方向')
...
def fmt_combined(a: dict, q: np.ndarray, src: str) -> tuple[str, str]:
    s_min = a['s'][-1]
    ...
    txt = (f"状態:   {stat}\n\n"
           ...
           f"\n\n──────────────────\n"
           f"入力: {src_label}" +
           (('\n' + '\n'.join(f"  {n:8s}: {v:+7.2f}°" for n, v in zip(NAMES, q)))
            if src == 'ros2' else ''))
    return txt, col
...
# [2] 楕円体 (初期)
draw_ellipsoid(ax_ell, a0)

# [3] 楕円投影 XY+XZ (初期)
draw_ellipses_2d(ax_ell2d, a0)
...
comb_t = ax_comb.text(0.05, 0.97, '', transform=ax_comb.transAxes,
                       va='top', fontsize=8.5)
ax_comb.set_title('メトリクス & 関節角度', fontsize=10)
txt0, col0 = fmt_combined(a0, q_init, _source)
comb_t.set_text(txt0)
comb_t.set_color(col0)
...
def animate(_):
    ...
    # [2] 楕円体 (3D)
    draw_ellipsoid(ax_ell, a)

    # [3] 楕円投影 XY+XZ (2D)
    draw_ellipses_2d(ax_ell2d, a)
    ...
    # [6] メトリクス + 関節角度 (統合)
    txt, col = fmt_combined(a, q, src)
    comb_t.set_text(txt)
    comb_t.set_color(col)
